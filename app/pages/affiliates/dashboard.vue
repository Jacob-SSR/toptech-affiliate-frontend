<template>
  <div class="flex flex-col md:flex-row h-screen bg-gray-100">
    ...
    <main class="flex-1 p-4 md:p-6 overflow-y-auto">
      ...
      >
        <h1 class="text-xl md:text-2xl font-bold mb-2 md:mb-0">ภาพรวม</h1>
      ...
      <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-4 mb-6">
        <div class="bg-white p-4 rounded shadow text-center">
          <p class="text-gray-500 text-sm">ค่าคอมมิชชั่น</p>
          <p class="text-xl font-bold">{{ dashboard?.commission || 0 }}</p>
        </div>
        <div class="bg-white p-4 rounded shadow text-center">
          <p class="text-gray-500 text-sm">ยอดรวมการคลิก</p>
          <p class="text-xl font-bold">{{ dashboard?.clicks || 0 }}</p>
        </div>
        <div class="bg-white p-4 rounded shadow text-center">
          <p class="text-gray-500 text-sm">ยอดรวมคำสั่งซื้อ</p>
          <p class="text-xl font-bold">{{ dashboard?.orders || 0 }}</p>
        </div>
        <div class="bg-white p-4 rounded shadow text-center">
          <p class="text-gray-500 text-sm">อัตราการเปลี่ยน</p>
      ...
      <div class="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-6">
        <div class="bg-white p-4 rounded shadow">
          <h2 class="font-bold mb-2 text-sm md:text-base">ยอดที่ถูกแนะนำ</h2>
          <client-only>
            <apexchart
              type="line"
              height="200"
...
onMounted(async () => {
  try {
    ...
    lineSeries.value = [
      { name: "คลิก", data: dashboard.value.chart.clicks },
      { name: "ค่าคอมมิชชั่น", data: dashboard.value.chart.commissions },
    ];

    donutOptions.value = {
      labels: ["ยอดคลิก", "คำสั่งซื้อ"],
    };
